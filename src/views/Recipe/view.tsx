...
export const RecipeView = () => {
	...
	useEffect(() => {
		...
		void getRecipe(id)
			.then(setRecipe)
			.catch((cause: unknown) =>
				setError(cause instanceof Error ? cause.message : 'Cannot load recipe')
			)
	}, [id])
			...
			{recipe && (
				<article className='recipe-detail'>
					...
					<div className='detail-meta'>
						...
						<div>
							<strong>{recipe.spiciness}</strong>
							<span>spice level</span>
						</div>
						{recipe.temperature !== null && (
							<div>
								<strong>{recipe.temperature}°F</strong>
					...
					<div className='detail-columns'>
						<section>
							...
							<h2>Tools</h2>
							<p className='preserve-lines'>{recipe.tools}</p>
						</section>
						<section>
							<h2>Directions</h2>
							<p className='preserve-lines directions'>{recipe.directions}</p>
						</section>
					</div>
				</article>
			)}
